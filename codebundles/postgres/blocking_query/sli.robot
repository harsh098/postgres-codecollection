...
*** Tasks ***
Total No of Blocking Queries in Postgres Cluster
    [Documentation]    Returns Total No Of Blocking Queries Running on the Postgres cluster
    [Tags]    postgres query inspection slowquery zalando
    ${query}=    Set Variable    SELECT COUNT(*) FROM (SELECT activity.pid,activity.usename,activity.query,blocking.pid AS blocking_id,blocking.query AS blocking_query FROM pg_stat_activity AS activity JOIN pg_stat_activity AS blocking ON blocking.pid\=ANY(pg_blocking_pids(activity.pid))) AS foo;
